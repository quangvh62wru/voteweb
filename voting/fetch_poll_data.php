...
$query = "SELECT `idIdol` FROM `chitietct`";
$statement = $connect->prepare($query);
$statement->execute();
$php_framework = $statement->fetchAll();

$total_poll_vote = get_total_vote($connect);
...
if($total_poll_vote > 0)
{
 ...
 {
  $query1 = "SELECT voteOfIdol FROM chitietct where idIdol=".$row[0];
 ...
 '.$percentage_vote.' % programmer like <b>'.$result2[0][0].'</b> PHP Framework
 </div>
 </div>
 </div>
 </div>
...
function get_total_vote( $connect )
{
 $connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
 $query = "SELECT `voteOfIdol` FROM `chitietct`";
 $statement = $connect->prepare($query);
 $statement->execute();
